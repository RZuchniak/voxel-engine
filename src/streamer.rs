        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            if self.procedural || !bootstrap {
                if let Some(bridge) = self.worker_bridge.as_mut() {
                    ...
                    if self.procedural {
                        // Workers exist but are still loading the wasm module, or their
                        // queue is full. Do NOT generate here: bootstrap requests the whole
                        // ring within the first few frames, so a synchronous fallback would
                        // run every chunk on the main thread before the workers ever wake up.
                        // Returning false leaves the coord unclaimed to retry next frame.
                        return false;
                    }
                }
            }
